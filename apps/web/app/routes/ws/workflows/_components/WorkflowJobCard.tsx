    ...
    <div className="flex items-center gap-1.5">
      {Object.entries(links).map(([label, url]) => (
        <a
          ...
          className="text-blue-500 hover:text-blue-400 hover:underline dark:text-blue-300"
        >
          {label}
        </a>
      ))}
...
function WorkflowJobJobSection({
  job,
}: {
  job: WorkspaceEngine["schemas"]["Job"];
}) {
  return (
    <JobDetailDialog job={job}>
      <div className="flex cursor-pointer items-center justify-between rounded-md p-2 hover:bg-accent">
...
function useWorkflowJobTemplate(workflowJob: WorkflowJob) {
  ...
  if (index >= workflow.jobs.length) return null;
