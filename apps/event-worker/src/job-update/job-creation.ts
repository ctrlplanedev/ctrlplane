...
export const createTriggeredRunbookJob = async (
  ...
  log.info(`Triger triggered runbook job ${runbook.name}`, {
    runbook,
    variableValues,
  });
  ...
  log.info(`Created triggered runbook job`, { jobId: job.id });
 ...
 * @param je
 */
export const onJobCompletion = async (db: Tx, je: schema.Job) => {
