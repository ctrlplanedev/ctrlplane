...
export const updateJob = async (
  ...
  console.log("updating job", jobId, data, metadata);
  ...
  console.log("job before update", jobBeforeUpdate);
  ...
  if (isJobFailure) await onJobFailure(db, updatedJob);

  const isJobCompletion =
    data.status === JobStatus.Successful &&
    jobBeforeUpdate.status !== JobStatus.Successful;
  if (isJobCompletion) await onJobCompletion(db, updatedJob);
