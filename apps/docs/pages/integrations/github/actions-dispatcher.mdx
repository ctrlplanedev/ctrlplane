...
and `target_config_deployment_id` to your workflows.
...
jobs:
  set-deployment-spec:
    ...
    steps:
      ...
      - id: ctrlplane
        uses: sizzldev/ctrlplane/github/get-job-inputs@main
        with:
          ...
          required_outputs: |
            release_version
            target_config_deployment_id
```

This example shows how the workflow retrieves specific values, such as
`release_version` and `target_config_deployment_id`, to be used in subsequent
