...
import type { EnvironmentVariableConfigType } from "@ctrlplane/validators/variables";
...
export const VariableEnvironmentInput: React.FC<
...
> = ({ value, onChange }) => {
  ...
  const { data: environments = [], isLoading } =
    api.environment.bySystemId.useQuery(system?.id ?? "", {
      enabled: system != null,
    });
  ...
  const selectedEnvironment = environments.find((r) => r.id === value);
          ...
          <Command shouldFilter={false}>
            ...
            <CommandList>
              ...
              {isSearchEmpty && !isLoading && (
                <CommandItem disabled>No environments found</CommandItem>
              )}
            </CommandList>
          </Command>
        </PopoverContent>
