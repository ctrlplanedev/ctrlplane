...
const useMetadataEquals = (form: RuleForm) => {
  ...
  const { data } = api.resource.relationshipRules.metadata.equals.useQuery({
    workspaceId,
    kind: targetKind === "" || targetKind == null ? undefined : targetKind,
    version:
      targetVersion === "" || targetVersion == null ? undefined : targetVersion,
  });

  return data ?? [];
};

export const TargetMetadataEqualsField: React.FC<{
  form: RuleForm;
  ...
  const selectedFormKey = form.watch(`targetMetadataEquals.${index}.key`);
