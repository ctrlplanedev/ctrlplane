...
// const schema = SCHEMA.policyRuleDenyWindowInsertSchema.extend({
...
export const CreateDenyRuleDialog: React.FC<{ workspaceId: string }> = ({
  ...
  const denyWindows = denyWindowsQ.data ?? [];
  console.log({ denyWindows });
  // const events = denyWindows.flatMap((denyWindow) => denyWindow.events);
  ...
  return (
    <DnDCalendar
      ...
      // onEventDrop={(event) => handleEventChange(event)}
