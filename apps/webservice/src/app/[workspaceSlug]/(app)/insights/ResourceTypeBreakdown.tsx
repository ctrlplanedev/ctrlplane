...
export const ResourceTypeBreakdown: React.FC<ResourceTypeBreakdownProps> = ({
  ...
  const { data: resources, isLoading } = api.resource.byWorkspaceId.list.useQuery({
    workspaceId,
    limit: 500,
  });
    ...
    <Card className="shadow-sm">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          ...
          <Tabs defaultValue="kind" onValueChange={setGroupBy} className="w-auto">
            <TabsList className="h-8">
              <TabsTrigger value="kind" className="text-xs px-3 h-7">Kind</TabsTrigger>
              <TabsTrigger value="provider" className="text-xs px-3 h-7">Provider</TabsTrigger>
              <TabsTrigger value="apiVersion" className="text-xs px-3 h-7">Version</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      </CardHeader>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                paddingAngle={1}
                label={({ name, percent }) => 
                  percent > 0.05 ? `${name.length > 12 ? name.substring(0, 12) + '...' : name}` : ''
                }
              >
              ...
              <Tooltip 
                ...
                contentStyle={{ borderRadius: '4px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)' }}
              />
              <Legend 
                layout="horizontal" 
