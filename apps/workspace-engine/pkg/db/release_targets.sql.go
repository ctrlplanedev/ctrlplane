...
type ListReleaseTargetsByDeploymentIDRow struct {
	...
	LatestJobStatus              NullJobStatus
	LatestJobMessage             pgtype.Text
	LatestJobReason              NullJobReason
	LatestJobCreatedAt           pgtype.Timestamptz
	LatestJobStartedAt           pgtype.Timestamptz
	LatestJobCompletedAt         pgtype.Timestamptz
	LatestJobUpdatedAt           pgtype.Timestamptz
	LatestJobExternalID          pgtype.Text
	LatestJobAgentID             pgtype.UUID
	LatestJobAgentConfig         []byte
	LatestJobDispatchContext     []byte
}
...
func (q *Queries) ListReleaseTargetsByDeploymentID(ctx context.Context, id uuid.UUID) ([]ListReleaseTargetsByDeploymentIDRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&i.LatestJobMessage,
			&i.LatestJobReason,
			&i.LatestJobCreatedAt,
			&i.LatestJobStartedAt,
			&i.LatestJobCompletedAt,
			&i.LatestJobUpdatedAt,
			&i.LatestJobExternalID,
			&i.LatestJobAgentID,
			&i.LatestJobAgentConfig,
			&i.LatestJobDispatchContext,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
