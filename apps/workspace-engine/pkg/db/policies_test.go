...
func TestDBPolicies_WithJsonDeploymentSelector(t *testing.T) {
	workspaceID, conn := setupTestWithWorkspace(t)
	...
	// Create a JSON deployment selector
	deploymentSelector := &oapi.Selector{}
	err = deploymentSelector.FromJsonSelector(oapi.JsonSelector{
	...
	if len(actualPolicies) != 1 {
		t.Fatalf("expected 1 policy, got %d", len(actualPolicies))
	}

	actualPolicy := actualPolicies[0]
	if len(actualPolicy.Selectors) != 1 {
		t.Fatalf("expected 1 selector, got %d", len(actualPolicy.Selectors))
	}

	actualSelector := actualPolicy.Selectors[0]
	if actualSelector.DeploymentSelector == nil {
		t.Fatalf("expected deployment selector to be non-nil")
	}

	// Validate the selector content
	jsonSelector, err := actualSelector.DeploymentSelector.AsJsonSelector()
	if err != nil {
...
func TestDBPolicies_WithCelEnvironmentSelector(t *testing.T) {
	workspaceID, conn := setupTestWithWorkspace(t)
	...
	// Create a CEL environment selector
	environmentSelector := &oapi.Selector{}
	celExpression := "environment.name == 'production'"
	err = environmentSelector.FromCelSelector(oapi.CelSelector{
		Cel: celExpression,
	})
	if err != nil {
	...
	actualSelector := actualPolicy.Selectors[0]
	if actualSelector.EnvironmentSelector == nil {
		t.Fatalf("expected environment selector to be non-nil")
	}

	// Validate the selector content
	celSelector, err := actualSelector.EnvironmentSelector.AsCelSelector()
	if err != nil {
...
func TestDBPolicies_WithJsonResourceSelectorContent(t *testing.T) {
	workspaceID, conn := setupTestWithWorkspace(t)
	...
	// Create a JSON resource selector
	resourceSelector := &oapi.Selector{}
	...
	actualSelector := actualPolicy.Selectors[0]
	if actualSelector.ResourceSelector == nil {
		t.Fatalf("expected resource selector to be non-nil")
	}

	// Validate the selector content
	jsonSelector, err := actualSelector.ResourceSelector.AsJsonSelector()
	if err != nil {
		t.Fatalf("expected JSON selector, got error: %v", err)
	}
...
func TestDBPolicies_WithMixedJsonAndCelSelectors(t *testing.T) {
	...
	// Create JSON deployment selector
	deploymentSelector := &oapi.Selector{}
	err = deploymentSelector.FromJsonSelector(oapi.JsonSelector{
		Json: map[string]interface{}{
			"type":  "name",
	...
	// Create CEL environment selector
	environmentSelector := &oapi.Selector{}
	envCelExpression := "environment.name in ['staging', 'production']"
	err = environmentSelector.FromCelSelector(oapi.CelSelector{
		Cel: envCelExpression,
	})
	if err != nil {
		t.Fatalf("failed to create CEL selector: %v", err)
	}

	// Create CEL resource selector
	resourceSelector := &oapi.Selector{}
	resCelExpression := "resource.metadata.critical == 'true'"
	err = resourceSelector.FromCelSelector(oapi.CelSelector{
		Cel: resCelExpression,
	})
	if err != nil {
		t.Fatalf("failed to create CEL selector: %v", err)
	}
	...
	// Validate deployment selector (JSON)
	if actualSelector.DeploymentSelector == nil {
		t.Fatalf("expected deployment selector to be non-nil")
	}
	jsonSelector, err := actualSelector.DeploymentSelector.AsJsonSelector()
	if err != nil {
		t.Fatalf("expected JSON deployment selector, got error: %v", err)
	}
	if jsonSelector.Json["type"] != "name" {
		t.Fatalf("expected deployment selector type 'name', got %v", jsonSelector.Json["type"])
	}
	if jsonSelector.Json["value"] != "api-service" {
		t.Fatalf("expected deployment selector value 'api-service', got %v", jsonSelector.Json["value"])
	}

	// Validate environment selector (CEL)
	if actualSelector.EnvironmentSelector == nil {
		t.Fatalf("expected environment selector to be non-nil")
	}
	envCelSelector, err := actualSelector.EnvironmentSelector.AsCelSelector()
	if err != nil {
		t.Fatalf("expected CEL environment selector, got error: %v", err)
	}
	if envCelSelector.Cel != envCelExpression {
		t.Fatalf("expected environment CEL expression %s, got %s", envCelExpression, envCelSelector.Cel)
	}

	// Validate resource selector (CEL)
	if actualSelector.ResourceSelector == nil {
		t.Fatalf("expected resource selector to be non-nil")
	}
	resCelSelector, err := actualSelector.ResourceSelector.AsCelSelector()
	if err != nil {
		t.Fatalf("expected CEL resource selector, got error: %v", err)
	}
	if resCelSelector.Cel != resCelExpression {
		t.Fatalf("expected resource CEL expression %s, got %s", resCelExpression, resCelSelector.Cel)
	}
}
