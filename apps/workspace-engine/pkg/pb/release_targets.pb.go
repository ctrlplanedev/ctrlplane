// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        v6.32.1
// source: release_targets.proto

package pb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	structpb "google.golang.org/protobuf/types/known/structpb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type Resource struct {
	state       protoimpl.MessageState `protogen:"open.v1"`
	Id          string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Name        string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Version     string                 `protobuf:"bytes,3,opt,name=version,proto3" json:"version,omitempty"`
	Kind        string                 `protobuf:"bytes,4,opt,name=kind,proto3" json:"kind,omitempty"`
	Identifier  string                 `protobuf:"bytes,5,opt,name=identifier,proto3" json:"identifier,omitempty"`
	CreatedAt   string                 `protobuf:"bytes,6,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"` // ISO8601 string or RFC3339 timestamp
	WorkspaceId string                 `protobuf:"bytes,7,opt,name=workspace_id,json=workspaceId,proto3" json:"workspace_id,omitempty"`
	ProviderId  string                 `protobuf:"bytes,8,opt,name=provider_id,json=providerId,proto3" json:"provider_id,omitempty"` // nullable
	// config can be a deeply nested object, so we use google.protobuf.Struct
	Config        *structpb.Struct  `protobuf:"bytes,9,opt,name=config,proto3" json:"config,omitempty"`
	LockedAt      string            `protobuf:"bytes,10,opt,name=locked_at,json=lockedAt,proto3" json:"locked_at,omitempty"`    // nullable, ISO8601 string or RFC3339 timestamp
	UpdatedAt     string            `protobuf:"bytes,11,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"` // nullable, ISO8601 string or RFC3339 timestamp
	DeletedAt     string            `protobuf:"bytes,12,opt,name=deleted_at,json=deletedAt,proto3" json:"deleted_at,omitempty"` // nullable, ISO8601 string or RFC3339 timestamp
	Metadata      map[string]string `protobuf:"bytes,13,rep,name=metadata,proto3" json:"metadata,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Resource) Reset() {
	*x = Resource{}
	mi := &file_release_targets_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Resource) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Resource) ProtoMessage() {}

func (x *Resource) ProtoReflect() protoreflect.Message {
	mi := &file_release_targets_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Resource.ProtoReflect.Descriptor instead.
func (*Resource) Descriptor() ([]byte, []int) {
	return file_release_targets_proto_rawDescGZIP(), []int{0}
}

func (x *Resource) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Resource) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Resource) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *Resource) GetKind() string {
	if x != nil {
		return x.Kind
	}
	return ""
}

func (x *Resource) GetIdentifier() string {
	if x != nil {
		return x.Identifier
	}
	return ""
}

func (x *Resource) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

func (x *Resource) GetWorkspaceId() string {
	if x != nil {
		return x.WorkspaceId
	}
	return ""
}

func (x *Resource) GetProviderId() string {
	if x != nil {
		return x.ProviderId
	}
	return ""
}

func (x *Resource) GetConfig() *structpb.Struct {
	if x != nil {
		return x.Config
	}
	return nil
}

func (x *Resource) GetLockedAt() string {
	if x != nil {
		return x.LockedAt
	}
	return ""
}

func (x *Resource) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

func (x *Resource) GetDeletedAt() string {
	if x != nil {
		return x.DeletedAt
	}
	return ""
}

func (x *Resource) GetMetadata() map[string]string {
	if x != nil {
		return x.Metadata
	}
	return nil
}

type Environment struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Id               string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Name             string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description      string                 `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	SystemId         string                 `protobuf:"bytes,4,opt,name=system_id,json=systemId,proto3" json:"system_id,omitempty"`
	ResourceSelector *structpb.Struct       `protobuf:"bytes,5,opt,name=resource_selector,json=resourceSelector,proto3" json:"resource_selector,omitempty"`
	CreatedAt        string                 `protobuf:"bytes,6,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *Environment) Reset() {
	*x = Environment{}
	mi := &file_release_targets_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Environment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Environment) ProtoMessage() {}

func (x *Environment) ProtoReflect() protoreflect.Message {
	mi := &file_release_targets_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Environment.ProtoReflect.Descriptor instead.
func (*Environment) Descriptor() ([]byte, []int) {
	return file_release_targets_proto_rawDescGZIP(), []int{1}
}

func (x *Environment) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Environment) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Environment) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Environment) GetSystemId() string {
	if x != nil {
		return x.SystemId
	}
	return ""
}

func (x *Environment) GetResourceSelector() *structpb.Struct {
	if x != nil {
		return x.ResourceSelector
	}
	return nil
}

func (x *Environment) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

type Deployment struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Id               string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Name             string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Slug             string                 `protobuf:"bytes,3,opt,name=slug,proto3" json:"slug,omitempty"`
	Description      string                 `protobuf:"bytes,4,opt,name=description,proto3" json:"description,omitempty"`
	SystemId         string                 `protobuf:"bytes,5,opt,name=system_id,json=systemId,proto3" json:"system_id,omitempty"`
	JobAgentId       *string                `protobuf:"bytes,6,opt,name=job_agent_id,json=jobAgentId,proto3,oneof" json:"job_agent_id,omitempty"`
	JobAgentConfig   *structpb.Struct       `protobuf:"bytes,7,opt,name=job_agent_config,json=jobAgentConfig,proto3,oneof" json:"job_agent_config,omitempty"`
	ResourceSelector *structpb.Struct       `protobuf:"bytes,8,opt,name=resource_selector,json=resourceSelector,proto3,oneof" json:"resource_selector,omitempty"`
	CreatedAt        string                 `protobuf:"bytes,9,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"` // ISO8601 string or RFC3339 timestamp
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *Deployment) Reset() {
	*x = Deployment{}
	mi := &file_release_targets_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment) ProtoMessage() {}

func (x *Deployment) ProtoReflect() protoreflect.Message {
	mi := &file_release_targets_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment.ProtoReflect.Descriptor instead.
func (*Deployment) Descriptor() ([]byte, []int) {
	return file_release_targets_proto_rawDescGZIP(), []int{2}
}

func (x *Deployment) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Deployment) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Deployment) GetSlug() string {
	if x != nil {
		return x.Slug
	}
	return ""
}

func (x *Deployment) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Deployment) GetSystemId() string {
	if x != nil {
		return x.SystemId
	}
	return ""
}

func (x *Deployment) GetJobAgentId() string {
	if x != nil && x.JobAgentId != nil {
		return *x.JobAgentId
	}
	return ""
}

func (x *Deployment) GetJobAgentConfig() *structpb.Struct {
	if x != nil {
		return x.JobAgentConfig
	}
	return nil
}

func (x *Deployment) GetResourceSelector() *structpb.Struct {
	if x != nil {
		return x.ResourceSelector
	}
	return nil
}

func (x *Deployment) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

type ReleaseTarget struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	ResourceId    string                 `protobuf:"bytes,2,opt,name=resource_id,json=resourceId,proto3" json:"resource_id,omitempty"`
	EnvironmentId string                 `protobuf:"bytes,3,opt,name=environment_id,json=environmentId,proto3" json:"environment_id,omitempty"`
	DeploymentId  string                 `protobuf:"bytes,4,opt,name=deployment_id,json=deploymentId,proto3" json:"deployment_id,omitempty"`
	CreatedAt     string                 `protobuf:"bytes,5,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	Environment   *Environment           `protobuf:"bytes,6,opt,name=environment,proto3" json:"environment,omitempty"`
	Deployment    *Deployment            `protobuf:"bytes,7,opt,name=deployment,proto3" json:"deployment,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReleaseTarget) Reset() {
	*x = ReleaseTarget{}
	mi := &file_release_targets_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReleaseTarget) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReleaseTarget) ProtoMessage() {}

func (x *ReleaseTarget) ProtoReflect() protoreflect.Message {
	mi := &file_release_targets_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReleaseTarget.ProtoReflect.Descriptor instead.
func (*ReleaseTarget) Descriptor() ([]byte, []int) {
	return file_release_targets_proto_rawDescGZIP(), []int{3}
}

func (x *ReleaseTarget) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ReleaseTarget) GetResourceId() string {
	if x != nil {
		return x.ResourceId
	}
	return ""
}

func (x *ReleaseTarget) GetEnvironmentId() string {
	if x != nil {
		return x.EnvironmentId
	}
	return ""
}

func (x *ReleaseTarget) GetDeploymentId() string {
	if x != nil {
		return x.DeploymentId
	}
	return ""
}

func (x *ReleaseTarget) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

func (x *ReleaseTarget) GetEnvironment() *Environment {
	if x != nil {
		return x.Environment
	}
	return nil
}

func (x *ReleaseTarget) GetDeployment() *Deployment {
	if x != nil {
		return x.Deployment
	}
	return nil
}

type ComputeReleaseTargetsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Environments  []*Environment         `protobuf:"bytes,1,rep,name=environments,proto3" json:"environments,omitempty"`
	Deployments   []*Deployment          `protobuf:"bytes,2,rep,name=deployments,proto3" json:"deployments,omitempty"`
	Resources     []*Resource            `protobuf:"bytes,3,rep,name=resources,proto3" json:"resources,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComputeReleaseTargetsRequest) Reset() {
	*x = ComputeReleaseTargetsRequest{}
	mi := &file_release_targets_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComputeReleaseTargetsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComputeReleaseTargetsRequest) ProtoMessage() {}

func (x *ComputeReleaseTargetsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_release_targets_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComputeReleaseTargetsRequest.ProtoReflect.Descriptor instead.
func (*ComputeReleaseTargetsRequest) Descriptor() ([]byte, []int) {
	return file_release_targets_proto_rawDescGZIP(), []int{4}
}

func (x *ComputeReleaseTargetsRequest) GetEnvironments() []*Environment {
	if x != nil {
		return x.Environments
	}
	return nil
}

func (x *ComputeReleaseTargetsRequest) GetDeployments() []*Deployment {
	if x != nil {
		return x.Deployments
	}
	return nil
}

func (x *ComputeReleaseTargetsRequest) GetResources() []*Resource {
	if x != nil {
		return x.Resources
	}
	return nil
}

var File_release_targets_proto protoreflect.FileDescriptor

const file_release_targets_proto_rawDesc = "" +
	"\n" +
	"\x15release_targets.proto\x12\tworkspace\x1a\x1cgoogle/protobuf/struct.proto\"\xe7\x03\n" +
	"\bResource\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x18\n" +
	"\aversion\x18\x03 \x01(\tR\aversion\x12\x12\n" +
	"\x04kind\x18\x04 \x01(\tR\x04kind\x12\x1e\n" +
	"\n" +
	"identifier\x18\x05 \x01(\tR\n" +
	"identifier\x12\x1d\n" +
	"\n" +
	"created_at\x18\x06 \x01(\tR\tcreatedAt\x12!\n" +
	"\fworkspace_id\x18\a \x01(\tR\vworkspaceId\x12\x1f\n" +
	"\vprovider_id\x18\b \x01(\tR\n" +
	"providerId\x12/\n" +
	"\x06config\x18\t \x01(\v2\x17.google.protobuf.StructR\x06config\x12\x1b\n" +
	"\tlocked_at\x18\n" +
	" \x01(\tR\blockedAt\x12\x1d\n" +
	"\n" +
	"updated_at\x18\v \x01(\tR\tupdatedAt\x12\x1d\n" +
	"\n" +
	"deleted_at\x18\f \x01(\tR\tdeletedAt\x12=\n" +
	"\bmetadata\x18\r \x03(\v2!.workspace.Resource.MetadataEntryR\bmetadata\x1a;\n" +
	"\rMetadataEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\xd5\x01\n" +
	"\vEnvironment\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12 \n" +
	"\vdescription\x18\x03 \x01(\tR\vdescription\x12\x1b\n" +
	"\tsystem_id\x18\x04 \x01(\tR\bsystemId\x12D\n" +
	"\x11resource_selector\x18\x05 \x01(\v2\x17.google.protobuf.StructR\x10resourceSelector\x12\x1d\n" +
	"\n" +
	"created_at\x18\x06 \x01(\tR\tcreatedAt\"\x98\x03\n" +
	"\n" +
	"Deployment\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x12\n" +
	"\x04slug\x18\x03 \x01(\tR\x04slug\x12 \n" +
	"\vdescription\x18\x04 \x01(\tR\vdescription\x12\x1b\n" +
	"\tsystem_id\x18\x05 \x01(\tR\bsystemId\x12%\n" +
	"\fjob_agent_id\x18\x06 \x01(\tH\x00R\n" +
	"jobAgentId\x88\x01\x01\x12F\n" +
	"\x10job_agent_config\x18\a \x01(\v2\x17.google.protobuf.StructH\x01R\x0ejobAgentConfig\x88\x01\x01\x12I\n" +
	"\x11resource_selector\x18\b \x01(\v2\x17.google.protobuf.StructH\x02R\x10resourceSelector\x88\x01\x01\x12\x1d\n" +
	"\n" +
	"created_at\x18\t \x01(\tR\tcreatedAtB\x0f\n" +
	"\r_job_agent_idB\x13\n" +
	"\x11_job_agent_configB\x14\n" +
	"\x12_resource_selector\"\x9c\x02\n" +
	"\rReleaseTarget\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x1f\n" +
	"\vresource_id\x18\x02 \x01(\tR\n" +
	"resourceId\x12%\n" +
	"\x0eenvironment_id\x18\x03 \x01(\tR\renvironmentId\x12#\n" +
	"\rdeployment_id\x18\x04 \x01(\tR\fdeploymentId\x12\x1d\n" +
	"\n" +
	"created_at\x18\x05 \x01(\tR\tcreatedAt\x128\n" +
	"\venvironment\x18\x06 \x01(\v2\x16.workspace.EnvironmentR\venvironment\x125\n" +
	"\n" +
	"deployment\x18\a \x01(\v2\x15.workspace.DeploymentR\n" +
	"deployment\"\xc6\x01\n" +
	"\x1cComputeReleaseTargetsRequest\x12:\n" +
	"\fenvironments\x18\x01 \x03(\v2\x16.workspace.EnvironmentR\fenvironments\x127\n" +
	"\vdeployments\x18\x02 \x03(\v2\x15.workspace.DeploymentR\vdeployments\x121\n" +
	"\tresources\x18\x03 \x03(\v2\x13.workspace.ResourceR\tresources2f\n" +
	"\x14ReleaseTargetService\x12N\n" +
	"\aCompute\x12'.workspace.ComputeReleaseTargetsRequest\x1a\x18.workspace.ReleaseTarget0\x01B\x19Z\x17workspace-engine/pkg/pbb\x06proto3"

var (
	file_release_targets_proto_rawDescOnce sync.Once
	file_release_targets_proto_rawDescData []byte
)

func file_release_targets_proto_rawDescGZIP() []byte {
	file_release_targets_proto_rawDescOnce.Do(func() {
		file_release_targets_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_release_targets_proto_rawDesc), len(file_release_targets_proto_rawDesc)))
	})
	return file_release_targets_proto_rawDescData
}

var file_release_targets_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_release_targets_proto_goTypes = []any{
	(*Resource)(nil),                     // 0: workspace.Resource
	(*Environment)(nil),                  // 1: workspace.Environment
	(*Deployment)(nil),                   // 2: workspace.Deployment
	(*ReleaseTarget)(nil),                // 3: workspace.ReleaseTarget
	(*ComputeReleaseTargetsRequest)(nil), // 4: workspace.ComputeReleaseTargetsRequest
	nil,                                  // 5: workspace.Resource.MetadataEntry
	(*structpb.Struct)(nil),              // 6: google.protobuf.Struct
}
var file_release_targets_proto_depIdxs = []int32{
	6,  // 0: workspace.Resource.config:type_name -> google.protobuf.Struct
	5,  // 1: workspace.Resource.metadata:type_name -> workspace.Resource.MetadataEntry
	6,  // 2: workspace.Environment.resource_selector:type_name -> google.protobuf.Struct
	6,  // 3: workspace.Deployment.job_agent_config:type_name -> google.protobuf.Struct
	6,  // 4: workspace.Deployment.resource_selector:type_name -> google.protobuf.Struct
	1,  // 5: workspace.ReleaseTarget.environment:type_name -> workspace.Environment
	2,  // 6: workspace.ReleaseTarget.deployment:type_name -> workspace.Deployment
	1,  // 7: workspace.ComputeReleaseTargetsRequest.environments:type_name -> workspace.Environment
	2,  // 8: workspace.ComputeReleaseTargetsRequest.deployments:type_name -> workspace.Deployment
	0,  // 9: workspace.ComputeReleaseTargetsRequest.resources:type_name -> workspace.Resource
	4,  // 10: workspace.ReleaseTargetService.Compute:input_type -> workspace.ComputeReleaseTargetsRequest
	3,  // 11: workspace.ReleaseTargetService.Compute:output_type -> workspace.ReleaseTarget
	11, // [11:12] is the sub-list for method output_type
	10, // [10:11] is the sub-list for method input_type
	10, // [10:10] is the sub-list for extension type_name
	10, // [10:10] is the sub-list for extension extendee
	0,  // [0:10] is the sub-list for field type_name
}

func init() { file_release_targets_proto_init() }
func file_release_targets_proto_init() {
	if File_release_targets_proto != nil {
		return
	}
	file_release_targets_proto_msgTypes[2].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_release_targets_proto_rawDesc), len(file_release_targets_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_release_targets_proto_goTypes,
		DependencyIndexes: file_release_targets_proto_depIdxs,
		MessageInfos:      file_release_targets_proto_msgTypes,
	}.Build()
	File_release_targets_proto = out.File
	file_release_targets_proto_goTypes = nil
	file_release_targets_proto_depIdxs = nil
}
