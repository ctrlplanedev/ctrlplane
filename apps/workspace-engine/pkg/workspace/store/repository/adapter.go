...
type MapStoreAdapter[T any] struct {
	store map[string]T
}

func (a *MapStoreAdapter[T]) typeAndKey(entity any) (typed T, key string, err error) {
	typed, ok := entity.(T)
	if !ok {
...
func (a *MapStoreAdapter[T]) Set(ctx context.Context, entity any) error {
	typed, key, err := a.typeAndKey(entity)
	if err != nil {
		return err
	}
...
func (a *MapStoreAdapter[T]) Unset(ctx context.Context, entity any) error {
	_, ok := entity.(T)
	if !ok {
		return fmt.Errorf("expected %T, got %T", *new(T), entity)
	}
	_, key, err := a.typeAndKey(entity)
	if err != nil {
		return err
