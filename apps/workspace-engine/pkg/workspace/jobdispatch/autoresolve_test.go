package jobdispatch

import (
	"context"
	"testing"
	"time"

	"workspace-engine/pkg/oapi"
	"workspace-engine/pkg/statechange"
	"workspace-engine/pkg/workspace/store"

	"github.com/stretchr/testify/require"
)

func TestAutoResolveDispatcher_DispatchJob_DefaultConfig(t *testing.T) {
	sc := statechange.NewChangeSet[any]()
	mockStore := store.New("test-workspace", sc)

	// Create a channel that we control for timing
	timeChannel := make(chan time.Time, 1)
	mockTimeFunc := func(d time.Duration) <-chan time.Time {
		return timeChannel
	}

	dispatcher := NewAutoResolveDispatcherWithTimeFunc(mockStore, mockTimeFunc)

	// Create and store a pending job
	job := &oapi.Job{
		Id:        "test-job-123",
		Status:    oapi.JobStatusPending,
		UpdatedAt: time.Now(),
	}
	mockStore.Jobs.Upsert(context.Background(), job)

	// Dispatch the job
	err := dispatcher.DispatchJob(context.Background(), job)
	require.NoError(t, err)

	// Job should still be pending before time elapses
	storedJob, exists := mockStore.Jobs.Get(job.Id)
	require.True(t, exists)
	require.Equal(t, oapi.JobStatusPending, storedJob.Status)

	// Trigger the time delay
	timeChannel <- time.Now()

	// Wait a bit for the goroutine to process
	time.Sleep(50 * time.Millisecond)

	// Job should now be successful
	storedJob, exists = mockStore.Jobs.Get(job.Id)
	require.True(t, exists)
	require.Equal(t, oapi.JobStatusSuccessful, storedJob.Status)
	require.NotNil(t, storedJob.Message)
	require.Contains(t, *storedJob.Message, "Auto-resolved by test dispatcher")
}

func TestAutoResolveDispatcher_DispatchJob_WithDelay(t *testing.T) {
	sc := statechange.NewChangeSet[any]()
	mockStore := store.New("test-workspace", sc)

	var receivedDuration time.Duration
	timeChannel := make(chan time.Time, 1)
	mockTimeFunc := func(d time.Duration) <-chan time.Time {
		receivedDuration = d
		return timeChannel
	}

	dispatcher := NewAutoResolveDispatcherWithTimeFunc(mockStore, mockTimeFunc)

	delaySeconds := 10
	job := &oapi.Job{
		Id:     "test-job-456",
		Status: oapi.JobStatusPending,
		JobAgentConfig: map[string]any{
			"delaySeconds": float64(delaySeconds),
		},
		UpdatedAt: time.Now(),
	}
	mockStore.Jobs.Upsert(context.Background(), job)

	err := dispatcher.DispatchJob(context.Background(), job)
	require.NoError(t, err)

	// Trigger the time delay
	timeChannel <- time.Now()
	time.Sleep(50 * time.Millisecond)

	// Verify the correct delay was used
	require.Equal(t, time.Duration(delaySeconds)*time.Second, receivedDuration)
}

func TestAutoResolveDispatcher_DispatchJob_FailureStatus(t *testing.T) {
	sc := statechange.NewChangeSet[any]()
	mockStore := store.New("test-workspace", sc)

	timeChannel := make(chan time.Time, 1)
	mockTimeFunc := func(d time.Duration) <-chan time.Time {
		return timeChannel
	}

	dispatcher := NewAutoResolveDispatcherWithTimeFunc(mockStore, mockTimeFunc)

	failureStatus := "failure"
	job := &oapi.Job{
		Id:     "test-job-789",
		Status: oapi.JobStatusPending,
		JobAgentConfig: map[string]any{
			"status": failureStatus,
		},
		UpdatedAt: time.Now(),
	}
	mockStore.Jobs.Upsert(context.Background(), job)

	err := dispatcher.DispatchJob(context.Background(), job)
	require.NoError(t, err)

	// Trigger the time delay
	timeChannel <- time.Now()
	time.Sleep(50 * time.Millisecond)

	// Job should now be failed
	storedJob, exists := mockStore.Jobs.Get(job.Id)
	require.True(t, exists)
	require.Equal(t, oapi.JobStatusFailure, storedJob.Status)
}

func TestAutoResolveDispatcher_DispatchJob_WithMessage(t *testing.T) {
	sc := statechange.NewChangeSet[any]()
	mockStore := store.New("test-workspace", sc)

	timeChannel := make(chan time.Time, 1)
	mockTimeFunc := func(d time.Duration) <-chan time.Time {
		return timeChannel
	}

	dispatcher := NewAutoResolveDispatcherWithTimeFunc(mockStore, mockTimeFunc)

	customMessage := "Custom test message"
	job := &oapi.Job{
		Id:     "test-job-msg",
		Status: oapi.JobStatusPending,
		JobAgentConfig: map[string]any{
			"message": customMessage,
		},
		UpdatedAt: time.Now(),
	}
	mockStore.Jobs.Upsert(context.Background(), job)

	err := dispatcher.DispatchJob(context.Background(), job)
	require.NoError(t, err)

	// Trigger the time delay
	timeChannel <- time.Now()
	time.Sleep(50 * time.Millisecond)

	// Job should have both custom message and auto-resolve message
	storedJob, exists := mockStore.Jobs.Get(job.Id)
	require.True(t, exists)
	require.NotNil(t, storedJob.Message)
	require.Contains(t, *storedJob.Message, customMessage)
	require.Contains(t, *storedJob.Message, "Auto-resolved by test dispatcher")
}

func TestAutoResolveDispatcher_SkipsTerminalState(t *testing.T) {
	terminalStates := []oapi.JobStatus{
		oapi.JobStatusSuccessful,
		oapi.JobStatusFailure,
		oapi.JobStatusCancelled,
	}

	for _, terminalStatus := range terminalStates {
		t.Run(string(terminalStatus), func(t *testing.T) {
			sc := statechange.NewChangeSet[any]()
			mockStore := store.New("test-workspace", sc)

			timeChannel := make(chan time.Time, 1)
			mockTimeFunc := func(d time.Duration) <-chan time.Time {
				return timeChannel
			}

			dispatcher := NewAutoResolveDispatcherWithTimeFunc(mockStore, mockTimeFunc)

			job := &oapi.Job{
				Id:        "test-job-terminal",
				Status:    terminalStatus,
				UpdatedAt: time.Now(),
			}
			mockStore.Jobs.Upsert(context.Background(), job)

			err := dispatcher.DispatchJob(context.Background(), job)
			require.NoError(t, err)

			// Trigger the time delay
			timeChannel <- time.Now()
			time.Sleep(50 * time.Millisecond)

			// Job should still have original status
			storedJob, exists := mockStore.Jobs.Get(job.Id)
			require.True(t, exists)
			require.Equal(t, terminalStatus, storedJob.Status)
		})
	}
}

func TestAutoResolveDispatcher_ParseConfig(t *testing.T) {
	tests := []struct {
		name          string
		jobConfig     map[string]any
		expectError   bool
		errorContains string
		expectedDelay int
		expectedFail  bool
	}{
		{
			name:          "empty config uses defaults",
			jobConfig:     nil,
			expectError:   false,
			expectedDelay: 5, // default
			expectedFail:  false,
		},
		{
			name: "custom delay",
			jobConfig: map[string]any{
				"delaySeconds": float64(30),
			},
			expectError:   false,
			expectedDelay: 30,
			expectedFail:  false,
		},
		{
			name: "failure status",
			jobConfig: map[string]any{
				"status": "failure",
			},
			expectError:   false,
			expectedDelay: 5,
			expectedFail:  true,
		},
		{
			name: "completed status",
			jobConfig: map[string]any{
				"status": "completed",
			},
			expectError:   false,
			expectedDelay: 5,
			expectedFail:  false,
		},
		{
			name: "invalid status",
			jobConfig: map[string]any{
				"status": "invalid",
			},
			expectError:   true,
			errorContains: "invalid status",
		},
		{
			name: "all options",
			jobConfig: map[string]any{
				"delaySeconds": float64(15),
				"status":       "failure",
				"message":      "Test message",
			},
			expectError:   false,
			expectedDelay: 15,
			expectedFail:  true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dispatcher := &AutoResolveDispatcher{}

			job := &oapi.Job{
				Id:             "test-job",
				JobAgentConfig: tt.jobConfig,
			}

			result, err := dispatcher.parseConfig(job)

			if tt.expectError {
				require.Error(t, err)
				if tt.errorContains != "" {
					require.Contains(t, err.Error(), tt.errorContains)
				}
			} else {
				require.NoError(t, err)

				expectedDelay := time.Duration(tt.expectedDelay) * time.Second
				actualDelay := dispatcher.getDelay(result)
				require.Equal(t, expectedDelay, actualDelay)

				expectedStatus := oapi.JobStatusSuccessful
				if tt.expectedFail {
					expectedStatus = oapi.JobStatusFailure
				}
				actualStatus := dispatcher.getFinalStatus(result)
				require.Equal(t, expectedStatus, actualStatus)
			}
		})
	}
}

func TestAutoResolveDispatcher_JobNotFound(t *testing.T) {
	sc := statechange.NewChangeSet[any]()
	mockStore := store.New("test-workspace", sc)

	timeChannel := make(chan time.Time, 1)
	mockTimeFunc := func(d time.Duration) <-chan time.Time {
		return timeChannel
	}

	dispatcher := NewAutoResolveDispatcherWithTimeFunc(mockStore, mockTimeFunc)

	// Don't add the job to the store
	job := &oapi.Job{
		Id:        "nonexistent-job",
		Status:    oapi.JobStatusPending,
		UpdatedAt: time.Now(),
	}

	// Dispatch should succeed (goroutine is spawned)
	err := dispatcher.DispatchJob(context.Background(), job)
	require.NoError(t, err)

	// Trigger the time delay
	timeChannel <- time.Now()
	time.Sleep(50 * time.Millisecond)

	// Job should not exist in store
	_, exists := mockStore.Jobs.Get(job.Id)
	require.False(t, exists)
}

func TestAutoResolveDispatcher_InProgressStatus(t *testing.T) {
	sc := statechange.NewChangeSet[any]()
	mockStore := store.New("test-workspace", sc)

	timeChannel := make(chan time.Time, 1)
	mockTimeFunc := func(d time.Duration) <-chan time.Time {
		return timeChannel
	}

	dispatcher := NewAutoResolveDispatcherWithTimeFunc(mockStore, mockTimeFunc)

	// Job starts as in_progress
	job := &oapi.Job{
		Id:        "test-job-in-progress",
		Status:    oapi.JobStatusInProgress,
		UpdatedAt: time.Now(),
	}
	mockStore.Jobs.Upsert(context.Background(), job)

	err := dispatcher.DispatchJob(context.Background(), job)
	require.NoError(t, err)

	// Trigger the time delay
	timeChannel <- time.Now()
	time.Sleep(50 * time.Millisecond)

	// Job should now be successful (in_progress is a valid state to auto-resolve)
	storedJob, exists := mockStore.Jobs.Get(job.Id)
	require.True(t, exists)
	require.Equal(t, oapi.JobStatusSuccessful, storedJob.Status)
}

