...
func TestJobRepository_BasicCRUD(t *testing.T) {
	...
	tenSecondsBefore := time.Now().Add(-time.Second * 10)
	fiveSecondsBefore := time.Now().Add(-time.Second * 5)
	oneSecondBefore := time.Now().Add(-time.Second * 1)
	oneSecondAfter := time.Now().Add(time.Second * 1)
	fiveSecondsAfter := time.Now().Add(time.Second * 5)
	tenSecondsAfter := time.Now().Add(time.Second * 10)
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, step := range test.steps {
				...
				for _, expectedJob := range step.expectedJobs {
					...
					assert.Equal(t, expectedJob.GetExternalID(), actualJob.GetExternalID())
					assert.Equal(t, expectedJob.GetJobAgentID(), actualJob.GetJobAgentID())
				}
			}
		})
	}
