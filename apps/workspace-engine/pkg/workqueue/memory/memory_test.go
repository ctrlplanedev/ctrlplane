...
func TestQueue_FilteredClaimAndRetry(t *testing.T) {
	...
	err = filtered.Enqueue(ctx, workqueue.EnqueueParams{
		WorkspaceID: workspaceID,
		Kind:        "otherkind",
		ScopeType:   "deployment",
		ScopeID:     uuid.NewString(),
...
func TestQueue_AckLeavesNewerPayload(t *testing.T) {
	...
	remaining, err := queue.Claim(ctx, workqueue.ClaimParams{
		BatchSize:     1,
		WorkerID:      "worker-b",
		LeaseDuration: time.Second,
	})
...
func TestQueue_ValidationErrors(t *testing.T) {
	...
	if err := queue.Enqueue(ctx, workqueue.EnqueueParams{WorkspaceID: "bad", Kind: "k"}); err == nil {
