...
// Config mirrors the PrometheusMetricProvider OpenAPI schema as a local type.
type Config struct {
	...
	RangeQuery     *RangeQuery     `json:"rangeQuery,omitempty"`
}
...
func NewPrometheusProvider(config *Config) (*PrometheusProvider, error) {
	if config.Address == "" {
...
func NewFromJSON(data json.RawMessage) (*PrometheusProvider, error) {
	var c Config
	if err := json.Unmarshal(data, &c); err != nil {
		return nil, fmt.Errorf("failed to unmarshal Prometheus provider: %w", err)
	}
	return NewPrometheusProvider(&c)
}
...
func resolveProviderTemplates(config *Config, providerCtx *provider.ProviderContext) *Config {
	resolved := &Config{
...
func resolveHeaders(headers []Header, providerCtx *provider.ProviderContext) []Header {
	if headers == nil {
		return nil
	}
	resolved := make([]Header, len(headers))
	for i, h := range headers {
		resolved[i] = Header{Key: h.Key, Value: providerCtx.Template(h.Value)}
	}
	return resolved
}
...
func buildQueryURL(config *Config, now time.Time) (string, error) {
...
func setHeaders(req *http.Request, config *Config, client *http.Client) error {
...
func setAuthHeader(req *http.Request, auth *Authentication, client *http.Client) error {
	if auth == nil {
		return nil
	}
...
func fetchOAuth2Token(ctx context.Context, oauth2 *OAuth2, client *http.Client) (string, error) {
	data := url.Values{
