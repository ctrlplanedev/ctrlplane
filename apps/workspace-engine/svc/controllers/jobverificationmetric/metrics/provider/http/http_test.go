...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			config:      &Config{URL: "http://example.com", Timeout: 10 * time.Second},
			wantMethod:  "GET",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if p.config.Method != tt.wantMethod {
				t.Errorf("expected method %s, got %s", tt.wantMethod, p.config.Method)
			}
			if p.config.Timeout != tt.wantTimeout {
				t.Errorf("expected timeout %v, got %v", tt.wantTimeout, p.config.Timeout)
			}
		})
	}
}
...
func TestMeasure(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			providerCtx:    &provider.ProviderContext{},
			wantStatusCode: 200,
			checkResult: func(t *testing.T, _ time.Time, data map[string]any) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			measuredAt, data, err := p.Measure(context.Background(), tt.providerCtx)
...
func TestMeasure_InvalidURL(t *testing.T) {
	p := &Provider{config: &Config{URL: "://invalid-url", Method: "GET", Timeout: 5 * time.Second}}
...
func TestMeasure_ContextCancellation(t *testing.T) {
	...
	p := &Provider{config: &Config{URL: server.URL, Method: "GET", Timeout: 5 * time.Second}}
...
func TestResolve(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			config: &Config{URL: "http://example.com/api/{{ .deployment.id }}", Method: "GET"},
			providerCtx: &provider.ProviderContext{
				Deployment: map[string]any{"id": "prod-deploy-123", "name": "prod-deployment"},
			},
		...
		{
			name:        "no templates",
			config:      &Config{URL: "http://example.com", Method: "GET", Body: "plain body"},
			providerCtx: &provider.ProviderContext{},
			wantURL:     "http://example.com",
			wantBody:    "plain body",
		},
		{
			...
			config: &Config{URL: "http://example.com/{{ .variables.service }}", Method: "GET"},
			providerCtx: &provider.ProviderContext{
				Variables: map[string]any{"service": "api"},
			},
...
func TestResolve_EmptyHeaders(t *testing.T) {
	...
	resolved := Resolve(config, &provider.ProviderContext{})
