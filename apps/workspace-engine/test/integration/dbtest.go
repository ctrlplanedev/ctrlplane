...
func newDBTestWorkspace(t *testing.T, options ...WorkspaceOption) *TestWorkspace {
	...
	ws := workspace.New(ctx, workspaceID,
		...
		workspace.WithStoreOptions(
			...
			store.WithDBRelationshipRules(ctx),
			store.WithDBPolicies(ctx),
			store.WithDBPolicySkips(ctx),
			store.WithDBUserApprovalRecords(ctx),
			store.WithDBDeploymentVariables(ctx),
			store.WithDBDeploymentVariableValues(ctx),
			store.WithDBWorkflows(ctx),
			store.WithDBWorkflowJobTemplates(ctx),
