...
```bash
curl -X POST https://app.ctrlplane.dev/api/v1/resources/{resourceId}/variables \
  -H "Authorization: Bearer $CTRLPLANE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PATCH https://app.ctrlplane.dev/api/v1/resources/{resourceId} \
  -H "Authorization: Bearer $CTRLPLANE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "metadata": {
...
```bash
curl -X DELETE https://app.ctrlplane.dev/api/v1/resources/{resourceId} \
  -H "Authorization: Bearer $CTRLPLANE_API_KEY"
```
...
```bash
curl https://app.ctrlplane.dev/api/v1/resources/{resourceId} \
  -H "Authorization: Bearer $CTRLPLANE_API_KEY"
```
...
- [Environments](./environments.md) - Group resources into environments
- [Selectors](./selectors.md) - Learn selector syntax for targeting resources
- [Resource Providers](../advanced/resource-providers.md) - Set up automated resource sync
