...
resource "ctrlplane_policy" "no_destroy_in_prod" {
  ...
  plan_validation_opa {
    ...
    description = "Block plans that delete or replace resources"
    rego        = <<-EOT
      package ctrlplane
...
<Warning>
  Invalid Rego (syntax errors) causes validation for that result to fail to
  evaluate. Test your policies with the
  [OPA playground](https://play.openpolicyagent.org/) or `opa eval` before
  saving them.
</Warning>
...
### Block resource deletions in a Terraform plan
