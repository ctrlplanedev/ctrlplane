...
| ----------- | ------ | -------- | -------------------------------------------------------------------------------------------------------- |
| `dependsOn` | string | Yes      | CEL expression to match upstream deployment(s) that must have a successful release before proceeding.    |
...
policies:
  - name: services-require-shared-lib
    ...
    rules:
      - deploymentDependency:
          dependsOn: deployment.metadata.type == "shared-library"
```
...
policies:
  - name: app-requires-infrastructure
    ...
    rules:
      - deploymentDependency:
          dependsOn: deployment.metadata.type == "infrastructure"
```
...
policies:
  - name: gateway-requires-services
    ...
    rules:
      - deploymentDependency:
          dependsOn: deployment.name in ["auth-service", "user-service", "billing-service"]
```
...
policies:
  - name: frontend-requires-backend
    ...
    rules:
      - deploymentDependency:
          dependsOn: deployment.metadata.layer == "backend"
```
...
Ctrlplane identifies all deployments matching the `dependsOn` CEL expression.
