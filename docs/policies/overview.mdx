...
4. **Priority** - Higher numeric value = higher priority. For the same rule
   type, the highest priority policy wins.
...
2. **Resolves rules by priority** - For each rule type, only the highest
   priority policy's rule is used. Different rule types are combined across
   policies.
3. **Applies rules** - Enforces each resolved rule type
