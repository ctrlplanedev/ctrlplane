...
export class EntityRefs {
    ...
    public takeEnvironments(
        count: number,
    ): Array<EnvironmentRef> {
    ...
    public getResourceLike(match: string): ResourceRef {
        return exactlyOne(
            this.resources.filter((res) =>
                matches(res.name, match) || matches(res.identifier, match)
            ),
        );
    }

    public takeDeployments(
        count: number,
    ): Array<DeploymentRef> {
...
function exactlyOne<T>(items: Array<T>): T {
    if (items.length !== 1) {
        throw new Error(
            `Expected exactly one item, but found ${items.length}.`,
        );
    }
    return items[0]!;
}
...
function takeRandom<T>(arr: Array<T>, n: number): Array<T> {
    if (n > arr.length) {
        throw new Error(
            `Cannot take ${n} elements, only ${arr.length} available.`,
        );
    }
